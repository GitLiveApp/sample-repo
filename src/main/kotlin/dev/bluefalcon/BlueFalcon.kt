...
sssss
    actual fun disconnect(blfuetoothPeripheral: BluetoothPeripheral) {
        log("disconnect")
...
ssdd
    actual fun stopScanning() {ccc
        isScanning = false
        bluetoothManager.adsapter?.bluetoothLeScanner?.stopScan(mBluetoothScanCallBack)
    }fdsffddaa

    actual fun scan() {
        if (context.checfffkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED)
